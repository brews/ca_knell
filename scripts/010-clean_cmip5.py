...
OUT_CMIP5_CONCAT_ZARR = f"{os.environ['CIL_SCRATCH_PREFIX']}/{os.environ['JUPYTERHUB_USER']}/{UID}/cmip5_concat.zarr"
OUT_CMIP5_DS_ZARR = f"{os.environ['CIL_SCRATCH_PREFIX']}/{os.environ['JUPYTERHUB_USER']}/{UID}/cmip5_ds.zarr"
...
SCENARIO_IDS = ("historical", "rcp45", "rcp85")
VARIABLE_IDS = ("tas", "tasmin", "tasmax")
VARIABLE_ID = "tas"
VERSION = 1.1  # tasmin and tasmax are 1.0
...
for variable_spec in variable_specs:
    for scenario_spec in scenario_specs:
        for model in scenario_spec.models:
            # NOTE: TO CREATE TEST DATA. Test data is only ACCESS1-0.
            if model != "ACCESS1-0":
                continue
...
# Below is usually more unstable.
...
cmip5_ds.to_zarr(OUT_CMIP5_DS_ZARR, mode="w")
