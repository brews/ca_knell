...
def _make_1998_2015_mean(ds: xr.Dataset) -> xr.Dataset:
    ...
    assert ds_sub["time"].size == 18, "'ds' should have 18 annual values when sliced from 1998 to 2015"
...
make_auffhammer_degreedays_1998_2015_mean = TransformationStrategy(
    preprocess=auffhammer_dds,
    postprocess=_make_1998_2015_mean
)


def _make_21yrmean(ds: xr.Dataset) -> xr.Dataset:
    ds = ds.rename({"time": "year"})  # Makes it consistent with time aggregations in other sectors. Not sure we want this or not.
